...
class AsFunctionPtrTest
{
    ...
    static int Main()
    {
        try{
            ...
            Console.WriteLine("Scenario 4 : Delegate marshaled as field in class with Sequential.");
            Class1_FuncPtrAsField4_Exp c4 = new Class1_FuncPtrAsField4_Exp();
            c4.verification = true;
            c4.dele = new Dele(CommonMethod);
            Assert.IsTrue(TakeDelegateAsFieldInClass_Exp(c4), "Delegate marshaled as field in class with Sequential.");
