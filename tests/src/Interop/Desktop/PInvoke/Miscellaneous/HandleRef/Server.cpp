...
extern "C" __declspec(dllexport) int WINAPI MarshalPointer_In(/*[in]*/int *pintValue, int stackGuard)
{
...
extern "C" __declspec(dllexport) int WINAPI MarshalPointer_InOut(/*[in,out]*/int *pintValue, int stackGuard)
{
    if(*pintValue != intManaged)
    {
...
extern "C" __declspec(dllexport) int WINAPI MarshalPointer_Out(/*[out]*/ int *pintValue, int stackGuard)
{
    *pintValue = intNative;
    if (stackGuard != expectedStackGuard)
    {
...
extern "C" __declspec(dllexport) int WINAPI TestNoGC(int *pintValue, GCCallback gcCallback)
{
